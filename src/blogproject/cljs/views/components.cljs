...
(defn welcome-title []
      (if-let [{:keys [full-name]} @m/session]
          (str "Welcome back, " full-name "!")
          "Welcome!"))
      ...
      [:div.fr.relative.hide-child
       (if session
        ...
        [:div
          ...
          [header-link (routes/page :signup) "Sign Up" "fr"]] )])
